...
var PERSONA_DEFS = {
  ...
  'student': {
    ...
    name:           '研学记录员',
    backendPersona: 'B',
    promptPrefix:   '[研学记录员视角：回答要方便边看边记，把观察任务、证据要点和“我的分析”自然融入内容；不要每次都套固定栏目。]',
  },
  'resident': {
    id:             'resident',
    name:           '研学记录员',
...
function buildStyledPrompt(rawInput, opts) {
  ...
  var PERSONA_TONE_MAP = {
    ...
    'student':  '像研学记录员一样帮助用户知道看什么、怎么记、这些证据如何形成解释；需要归纳时使用“我的分析：”，但不要固定套“观察任务/笔记要点”栏目。',
    'resident': '像研学记录员一样帮助用户知道看什么、怎么记、这些证据如何形成解释；需要归纳时使用“我的分析：”，但不要固定套“观察任务/笔记要点”栏目。',
    'B':        '像研学记录员一样帮助用户知道看什么、怎么记、这些证据如何形成解释；需要归纳时使用“我的分析：”，但不要固定套“观察任务/笔记要点”栏目。',
  ...
  parts.push([
    ...
    '不要使用固定模板小标题，尤其不要把回答分成重要性、后续观察建议等段落；需要解释含义时写“我的分析：”，这行后面换行输出你的总结，不要写“说明了什么”。',
    '连续bullet不超过4个。',
    '---',
  ].join('\n'))
