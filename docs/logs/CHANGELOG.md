...
## 2026-08-11 真机验收与发布文档收口

- 用户已确认报告保存、离厅后建议上下文、停止生成后的建议恢复和键盘遮挡四项真机回归正常；当前 `main` 基线可作为新体验版候选，本轮不需要重新部署后端。
- 新增仓库内权威 `docs/wechat-release-runbook.md`，覆盖变更类型判断、精确 SHA/上传边界门禁、体验版上传与成员分发、正式上线前检查、审核发布、上线后 30 分钟/24 小时/日常观察、前端与后端/数据分层回退和发布记录模板。
- 中英文 README 将阶段更新为体验版上传准备，删除“本轮仍在处理/仍待四项真机复测”和 TTS 未真机验证等过期表述；纠正英文路线排序、展厅目录和报告展示与中文实现不一致的问题，并改为引用仓库内手册。
- 项目材料中的旧上线准备文件仅保留跨仓库导航，服务器命令以后端权威维护手册为准，避免多份发布命令漂移。
- 验证通过：`npm run test:all` 的 16 组检查全部通过，发布预检为 65 个源码文件、118 个上传包文件 / 507 KiB、正式 HTTPS API；Markdown 相对链接、UTF-8 替换字符、公开文档旧 IP/密钥形态和 `git diff --check` 检查均通过。

## 2026-08-11 真机报告、建议条与键盘布局修复

- 报告页删除低价值的“下一步怎么看”展示，替换为“保存本次记录”：把报告标题、观察身份、展品/问题/时长和后端提炼的记录摘要复制为纯文本参观笔记，剪贴板失败时明确提示重试；旧后端 `exploration_guidance` 保持接口兼容但不再驱动当前页面。
- 修复展品建议残留：普通/恢复入厅会清空 current 与 pending 展品上下文；展厅列表入口、自定义返回、系统返回/页面卸载均执行兜底清理，离厅排队同步 `current_exhibit_id: null`，只有显式从展品详情返回时继续展品讨论。
- 建议请求新增 `localTourId + sessionId + hall + exhibit + seq` owner 校验，并在页面隐藏/卸载时使旧请求失效；停止等待首段或停止流式输出后恢复已有建议，无缓存时只请求一次。
- 真机键盘抬升由输入栏 `margin-bottom` 参与 flex 布局，不再使用 transform 覆盖回答和固定 `176rpx` 补偿；消息列表允许收缩，键盘布局提交后执行稳定底部滚动。
- 新增报告保存、旧 guidance 忽略、旧上下文响应隔离、两种停止状态、展品上下文清理和 `300px - 20px = 280px` 键盘占位回归；`npm run test:all` 的 16 组检查全部通过，发布预检检查 65 个源码文件，上传包边界为 118 个文件 / 507 KiB。微信真机四场景仍需用新体验版复测。

## 2026-08-11 体验版上传包收紧

- 本机 `project.config.json` 的 `packOptions.ignore` 改为显式排除协作文档、测试脚本、npm/Git 文件、构建产物及本地项目配置；微信上传包只保留小程序运行所需入口、API、资源、组件、常量、页面、状态、样式和工具目录。
- 本机关闭 `uploadWithSourceMap`，发布预检强制保持关闭，避免 504 KiB 源树边界之外再上传源码映射；本地开发调试不受影响。
- 发布预检新增 fail-closed 上传边界：`packOptions.ignore` 必须与已审核规则完全一致；缺少或额外增加规则、排除类型与根条目实际类型不一致、根目录出现未归类条目、运行目录出现未审核扩展名或符号链接时直接非零退出；通过时只报告运行包文件数和体积，不输出 AppID。
- 中英文 README 删除具体模型供应商、额度状态和语音取值，改为指向私有运维配置；英文服务器运行状态同步为当前 systemd、日志留存与 PostgreSQL 备份现状。
- 微信开发者工具关闭后确认无残留进程；`npm run test:all` 的 16 组检查全部通过，发布预检检查 65 个运行文本源码文件，收紧后的上传包边界为 118 个文件、504 KiB。
- 故障注入验证通过：临时缺少 `file:README.md` 排除项、加入未知顶层文件、误排除运行入口 `app.js`、用 `file` 规则误排除同名目录、在 `pages` 放入 `.txt` 文件、单独排除运行页面文件、重新开启 source map 上传时，预检均非零退出并指出具体问题，随后全部恢复干净配置。

## 2026-08-10 报告页 422 契约修复

- 定位报告页打不开并非报告接口故障：路线页把九厅 route step 的 `short`、`exhibitCount`、`exhibitCountKnown` 写入完整恢复快照，旧生产后端严格 schema 未声明三字段，产生 27 条 `extra_forbidden`；报告页在生成前同步快照时被 422 阻断。
- 保留三项路线恢复信息，配套后端显式接收受约束字段；客户端不通过删字段绕过，也不清空已受影响设备的 pending 快照。新版后端部署后旧快照可继续补传，无需用户清缓存。
- 修复同步并发放大：多个页面加入同一 in-flight PATCH 时，相同待同步快照的非成功结果直接共享；只有飞行期间合入了不同的新状态才继续 flush。422 仍保持可见并保留完整快照，不会被静默忽略。
- `test-tour-sync` 新增真实 route step 字段契约、三个并发等待者只发一次失败 PATCH、409 刷新版本后遇到 422 不重放最终请求，以及飞行期间合入不同新状态仍继续同步的回归；`npm run test:all` 16 组检查与 64 文件发布预检通过。
